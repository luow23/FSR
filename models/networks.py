...
class EfficientNet(nn.Module):
    ...
    def forward(self, input_):
        ...
        f1_ = F.interpolate(f1, size=(64, 64), mode='bilinear', align_corners=True)
        f2_ = F.interpolate(f2, size=(64, 64), mode='bilinear', align_corners=True)
        f3_ = F.interpolate(f3, size=(64, 64), mode='bilinear', align_corners=True)
        f4_ = F.interpolate(f4, size=(64, 64), mode='bilinear', align_corners=True)
...
class Resnet50(nn.Module):
    ...
    def forward(self, input_):
        ...
        f3_ = F.interpolate(out3, size=(64, 64), mode='bilinear', align_corners=True)
        f4_ = F.interpolate(out4, size=(64, 64), mode='bilinear', align_corners=True)
        avg_filter = torch.nn.AvgPool2d(3, 1, 1)
        f3_ = avg_filter(f3_)
        f4_ = avg_filter(f4_)
