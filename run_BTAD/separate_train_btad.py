...
class Model(object):
    def __init__(self, opt):
        ...
        if self.opt.model_name == "VIT_dir" or self.opt.model_name == "VIT_dir_noskip":
            self.save_root = f"./result_separate_BTAD/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt. in_lay_num}"
        else:
            self.save_root = f"./result_separate_BTAD/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt.in_lay_num}_shuffle_ratio={opt.shuffle_ratio}"
    ...
    def train(self):
        ...
        for epoch in range(self.opt.iter, self.opt.niter):
            ...
            if self.opt.is_log == True:
                ...
                loss_data.to_csv(os.path.join(self.save_root, f'{self.class_name}_train_loss.csv'), mode='a', header=False, index=False)
            # if (epoch+1) == 300:
            #     print('save model')
            ...
            if (epoch + 1)%10  == 0:
                ...
                if self.opt.is_log == True:
                    ...
                    data.to_csv(os.path.join(self.save_root, f'{self.class_name}_train_auroc.csv'), mode='a', header=False, index=False)
                auc_roc = x1+x2
                if auc_roc > auc_now:
                    # no_update_num = 0
                    auc_now = auc_roc
...
if __name__ == '__main__':
    from separate_config_btad import DefaultConfig
    opt = DefaultConfig()
    ...
    for classname in BTAD_CLASS_NAMES:
        ...
        opt.train_dataset = BTADDataset(dataset_path=opt.data_root, class_name=opt.class_name, is_train=True)
