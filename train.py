...
class Model(object):
    def __init__(self, opt):
        ...
        if self.opt.model_name == "VIT_dir" or self.opt.model_name == "VIT_dir_noskip":
            self.save_root = f"./result/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt. in_lay_num}"
        else:
            self.save_root = f"./result/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt.in_lay_num}_shuffle_ratio={opt.shuffle_ratio}"
...
if __name__ == '__main__':
    ...
    # 保存到 CSV
    output_path = f"single+class_auroc_results_{opt.shuffle_ratio}.csv"
