...
class Model(object):
    def __init__(self, opt):
        ...
        if self.opt.model_name == "VIT_dir":
            self.save_root = f"./result_uniad/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt. in_lay_num}"
            # df.to_csv(os.path.join(self.save_root, 'train.csv'), index=False)
        else:
            self.save_root = f"./result_uniad/{opt.model_name}_{opt.backbone_name}_k={opt.k}_inpaint_num={opt.in_lay_num}_shuffle_ratio={opt.shuffle_ratio}_avg_Ture_2+3+4_add_jitter_scale30"
        if self.opt.is_log == True:
            os.makedirs(self.save_root, exist_ok=True)
            df_loss = pd.DataFrame(columns=['Epoch', 'Train loss'])
            df_loss.to_csv(os.path.join(self.save_root, 'train_loss.csv'), index=False)
            df_auroc = pd.DataFrame(columns=['Epoch', 'Image AUROC', 'Pixel AUROC'])
            df_auroc.to_csv(os.path.join(self.save_root, 'train_auroc.csv'), index=False)
    ...
    def train(self, class_rocauc):
        ...
        for epoch in range(self.opt.iter, self.opt.niter):
            ...
            if self.opt.is_log == True:
                ...
                loss_data.to_csv(os.path.join(self.save_root, 'train_loss.csv'), mode='a',header=False,index=False)
            ...
            if (epoch + 1) % 10  == 0:
                ...
                if self.opt.is_log == True:
                    ...
                    data.to_csv(os.path.join(self.save_root, 'train_auroc.csv'), mode='a',header=False,index=False)
                auc_roc = x1+x2
                if auc_roc > auc_now:
                    # no_update_num = 0
                    auc_now = auc_roc
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        opt.testloader_list = [DataLoader(i, batch_size=1, shuffle=False,  num_workers = 4) for i in test_datalset_list]

        class_rocauc = {name: (0, 0, 0, 0) for name in class_names}

        model = Model(opt)
        class_rocauc = model.train(class_rocauc)  # 运行后更新 class_rocauc

        img_roc_current = [class_rocauc[name][0] for name in class_names]
        pixel_roc_current = [class_rocauc[name][1] for name in class_names]
    ...
    # 保存到 CSV
    output_path = f"unified+class_auroc_results_{opt.shuffle_ratio}.csv"
